...
async function loadPosts() {
    try {
        ...
        document.getElementById('posts-container').innerHTML = '<p>Error loading cosmic explorations.</p>';
    }
}
...
function displayPosts() {
    ...
    if (postsToShow.length === 0) {
        container.innerHTML = '<p>No explorations found yet. Check back soon for new cosmic discoveries!</p>';
        return;
    }
    ...
    container.innerHTML = postsToShow.map(post => `
        <article class="post-card">
            <a href="posts/${post.slug}.html">
                <h3>${post.title}</h3>
                <p class="meta">${formatDate(post.date)}${post.arxiv ? ` · arXiv: ${post.arxiv}` : ''}</p>
                <p>${post.excerpt}</p>
            </a>
        </article>
    `).join('');
